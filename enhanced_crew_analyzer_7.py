...
def summarize_patient_profile(profile: Dict) -> str:
    """Create simple patient summary for LLM."""
    lines = []
    ...
    # Medical conditions - simple yes/no
    lines.append("  Medical Conditions:")
    lines.append(f"    Diabetes: {'Yes' if profile['Diabetes'] == 1 else 'No'}")
    lines.append(f"    Hypertension: {'Yes' if profile['Hypertension'] == 1 else 'No'}")
    lines.append(f"    Heart Disease: {'Yes' if profile['Heart_Disease'] == 1 else 'No'}")
    lines.append(f"    COPD: {'Yes' if profile['COPD'] == 1 else 'No'}")
    lines.append(f"    Arthritis: {'Yes' if profile['Arthritis'] == 1 else 'No'}")
    lines.append(f"    Depression: {'Yes' if profile['Depression'] == 1 else 'No'}")
    lines.append(f"    Smoker: {'Yes' if profile['Smoker'] == 1 else 'No'}")
    
    return "\n".join(lines)_data['HRV'] < 20]
